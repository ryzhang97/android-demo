package com.ryzhang.android_demo.view.wheel.interfaces;

import com.ryzhang.android_demo.bean.address.AddressBean;
import com.ryzhang.android_demo.bean.address.CityChildsBean;
import com.ryzhang.android_demo.bean.address.CountyChildsBean;

/**
 * 作者：liji on 2017/11/16 10:06
 * 邮箱：lijiwork@sina.com
 * QQ ：275137657
 */

public abstract class OnCityItemClickListener {

    /**
     * 当选择只显示省份的选择器时，需要覆盖此方法
     *
     * @param province
     */
    public void onSelected(AddressBean province) {

    }

    /**
     * 当选择省市两级选择器时，需要覆盖此方法
     *
     * @param province
     * @param city
     */
    public void onSelected(AddressBean province, CityChildsBean city) {

    }

    /**
     * 当选择省市区三级选择器时，需要覆盖此方法
     *
     * @param province
     * @param city
     * @param district
     */
    public void onSelected(AddressBean province, CityChildsBean city, CountyChildsBean district) {

    }

    /**
     * 取消
     */
    public void onCancel() {

    }
}
